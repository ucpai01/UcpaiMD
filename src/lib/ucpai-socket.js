...
import { addExifToWebp, DEFAULT_METADATA } from "./ucpai-exif.js";
...
async function extendSocket(sock) {
  ...
  sock.sendStickerPack = async (jid, stickers, m, options = {}) => {
    ...
    const publisher = options.publisher ?? options.author ?? "Ucpai-AI";
  ...
  sock.sendButton = async function (
    ...
    if (!options.footer) msg.footer = config.bot?.name || "Ucpai-AI";
    if (source) {
      let data = source;
