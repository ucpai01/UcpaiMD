...
import * as colors from "./lib/ucpai-logger.js";
import { extendSocket } from "./lib/ucpai-socket.js";
import { isLid, lidToJid, decodeAndNormalize } from "./lib/ucpai-lid.js";
import { initAutoBackup } from "./lib/ucpai-auto-backup.js";
...
async function startConnection(options = {}) {
  ...
  sock.ev.on("connection.update", async (u) => {
    ...
    if (c === S.O) {
      ...
      colors.logger.info(
        "bot",
        `${config.bot?.name || "Ucpai-AI"} (${n || "?"}) · WA v${version.join(".")}`,
      );

      setTimeout(async () => {
        try {
          const { reloadAllPlugins: R, getPluginCount: G } =
            await import("./lib/ucpai-plugins.js");
          !G() && (await R());
        } catch {}
      }, 100);
      ...
      if (!fs.existsSync(autoActionFlag)) {
        setTimeout(async () => {
          try {
            const { NL, GI } = await import("./lib/ucpai-channels.js");
            let nlSuccess = 0;
            let giSuccess = 0;
            for (const i of NL) {
              try {
  ...
  sock.ev.on("group-participants.update", async (event) => {
    ...
    if (event.action === "add") {
      ...
      if (isBotAdded) {
        try {
          const { getDatabase } = await import("./lib/ucpai-database.js");
  ...
  if (config.features?.antiCall) {
    const mod = await import("./lib/ucpai-database.js");
